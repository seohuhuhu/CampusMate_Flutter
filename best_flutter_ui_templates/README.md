...
| **🔥 매너 온도 배지** | 작성자 닉네임 옆에 신뢰도 온도 표시 (색상으로 단계 구분) — 탭 시 상세 팝업 |
| **🔥 내 매너 온도 아이콘** | 앱바 우상단 불꽃 아이콘에 현재 온도 뱃지 표시 — 탭 시 상세 팝업 (항상 노출) |
| **🔥 매너 온도 팝업** | 점수 올리는 방법(초록) + 점수 떨어지는 경우(빨강) 한 화면에 표시 |
...
| 스터디룸 예약을 **사전** 취소 | +1° | 다른 유저에게 자리 양보 |

### 점수 하락 구조

| 행동 | 점수 | 근거 |
|---|---|---|
| 참가자가 있는 밥친구 글 삭제 | -3° | 참가자에게 노쇼와 동일한 피해 |
| 스터디룸 예약 **당일** 취소 | -2° | 다른 유저의 당일 예약 기회 박탈 |

> 💡 삭제 시 참가자가 있으면 경고 팝업으로 패널티를 사전 고지합니다.
...
- **앱바 우상단 🔥 아이콘** — 현재 온도 숫자 뱃지 상시 표시, 탭 시 팝업 (취향 설정 여부 무관)
- **게시글 카드 닉네임 옆 배지** — 작성자 온도를 색상으로 즉시 확인, 탭 시 해당 유저 온도 팝업
- **매너 온도 팝업** — 점수 올리는 방법(🟢 초록) + 점수 떨어지는 경우(🔴 빨강) 함께 표시
- **삭제 경고 팝업** — 참가자가 있는 글 삭제 시 `-3°` 패널티 사전 고지
